...
def prompt_output_to_eval_dict(init_dict, qprompt, result_text, manual_check=False):
    ...
    if manual_check:
        ...
        if gt_type == "simple" and correct_type:
            ...
            if correct_type:
                decomp_correct = (input("Is the decomposition correct? ('T' for yes) >>") == 'T')
...
def process_prompt_outputs(
    ...
    stats = {
        ...
        "correct_type_percent": {k: num_correct_type[k] * 100.0 / num_qs[k] for k in num_qs.keys()},
    }
    if manual_check:
        stats['num_correct_decomp'] = dict(num_correct_decomp)
        stats['correct_decomp_percent'] = {k: num_correct_decomp[k] *100.0 / num_qs[k] for k in num_qs.keys()}
...
def qdata_to_print_prompt_v0(all_qdata, qdecompose, num_each=3, include_simple=False, shuffle=True):
    prompt_list = []
    if include_simple:
        i = 0
        for qd in all_qdata:
...
Can this be decomposed: No.""".format(
    ...
    comp_dec = {k: v for k, v in qdecompose.items() if v['question_type'] == "composition"}
    int_dec = {k: v for k, v in qdecompose.items() if v['question_type'] == "intersection"}
...
"""
    ...
    for qid in [53823, 36854, 16043, 53774, 55950, 40308, 22726, 28673, 4484, 53887, 29563]:
